...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if(!this.isValidNode()){
      throw new InvalidNodeException("not valid");
    }else{
      if(c<1){
        c = 1;
      }
      ...
      if(c == 1){
        ...
        SibTreeNode preNode = this.firstChild;
        for(int i=1; i<c-1; i++){
          preNode = preNode.nextSibling;
        }
        node.nextSibling = preNode.nextSibling;
        preNode.nextSibling = node;
      }
      this.myTree.size++;
    }
  }
      ...
      if(this.children() == 0){
        if(this.parent == null){
          ...
          if(this.parent.firstChild == this){
            ...
            SibTreeNode prevNode = this.parent.firstChild;
            while(prevNode.nextSibling != this){
