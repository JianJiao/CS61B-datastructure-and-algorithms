...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART II HERE.
		...
		if(this.firstChild!=null){
			if(c<=1){
				...
				SibTreeNode before = this.firstChild;
				int i = 0;
				while(i<c-2){
	...
	private void insertAfter(SibTreeNode newNode){
		newNode.nextSibling = this.nextSibling;
		this.nextSibling = newNode;
	}

	private void insertFront(SibTreeNode newNode){
    ...
    // FILL IN YOUR SOLUTION TO PART III HERE.
		...
		if(this.firstChild!=null){
			...
			if(this.parent==null){
				...
				if(this.parent.firstChild.equals(this)){
					this.parent.firstChild = this.nextSibling;
					this.valid = false;
					this.myTree.size--;
				}else{
					SibTreeNode pointer = this.parent.firstChild;
					while(!pointer.nextSibling.equals(this)){
						pointer = pointer.nextSibling;	
					}
					pointer.nextSibling = this.nextSibling;
					this.valid = false;
					this.myTree.size--;
				}
			}
