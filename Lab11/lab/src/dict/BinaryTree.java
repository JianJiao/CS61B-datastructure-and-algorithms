...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
    if(node == null){
      ...
      if(key.compareTo(nodeKey) == 0){
        ...
        if(node.leftChild != null){
          return this.findHelper((Comparable)key, node.leftChild);
        }else{
          return null;
        }
      }else{
        if(node.rightChild != null){
          return this.findHelper((Comparable)key, node.rightChild);
        }else{
          return null;
        }
      }
  ...
  public Entry remove(Object key) {
    BinaryTreeNode node = this.findHelper((Comparable)key, this.root);
    ...
    if(node == null){
      ...
      if(!(node.leftChild !=null && node.rightChild !=null)){
        detachNodeWithNoOrOneChild(node);
        node.setNull();
      }else{
        BinaryTreeNode replaceNode = nextSmallest(node);
        detachNodeWithNoOrOneChild(replaceNode);
        changeParentsPointer(node, replaceNode);
  ...
  protected BinaryTreeNode nextSmallest(BinaryTreeNode node){
    BinaryTreeNode returnNode = node.rightChild;
  ...
  protected void detachNodeWithNoOrOneChild(BinaryTreeNode node){
  ...
  protected void changeParentsPointer(BinaryTreeNode node, BinaryTreeNode replaceNode){
    if( node == this.root ){
      ...
      // System.out.println("node parent right child " + node.parent.rightChild);
      // System.out.println("node entry key " + node.entry.key);
