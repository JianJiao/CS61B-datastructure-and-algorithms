...
public class Sorts {
  ...
  public static int[] countingSort(int[] keys, int whichDigit) {
    ...
    for(int i=0; i<keys.length; i++){
      targets[i] = keys[i];
      targets[i] = targets[i]>>(4*whichDigit);
      targets[i] = targets[i] & 15;
      buckets[targets[i]]++;
    }
    int scan[] = new int[16];
    scan[0] = 0;
    for(int i=1; i< buckets.length; i++){
      scan[i] = scan[i-1] + buckets[i-1];
    }
    int output[] = new int[keys.length];
    for(int i=0; i<keys.length; i++){
      output[scan[targets[i]]] = keys[i];
      scan[targets[i]]++;
    }
    return output;
  }
  ...
  public static void main(String[] args) {
    ...
    int tkeys[] = {1, 3, 3, 2, 4, 5, 6, 1, 4, 222, 432, 6543, 3445, 999};
    int output[] = Sorts.countingSort(tkeys, 0);
