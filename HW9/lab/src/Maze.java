...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    int cellNum = horiz * vert;
    int wallNum = horiz * (vert -1) + vert * (horiz-1);
    ...
    if (horiz > 1) {
      ...
      for (i = 0; i < horiz - 1; i++) {
        for (j = 0; j < vert; j++) {
          vWalls[i][j] = true;
          //walls[wallIdx].setWall(1, i, j);
          walls[wallIdx] = new Wall(1, i, j);
          wallIdx++;
        }
      }
    }
     ...
     **/
    cells = new DisjointSets(cellNum);

    int w = wallNum;
    Wall tmp;
    while(w > 1){
      int select = randInt(w);
      tmp = walls[select];
      walls[select] = walls[w-1];
      walls[w-1] = tmp;
      w--;
    }

    int cellIdx1, cellIdx2;
    for(int p=0; p<wallNum; p++){
      // you made mistakes here
      // the cellIdx is the index of the cell, not the wall, 
      // there are only so many cells, no diff about vert and horiz
      // you may get same cell index from horz and vert walls
      // should first get the index of 2*2 arrays, then the whole index
      if(walls[p].hv==0){
        cellIdx1 = walls[p].x * vert + walls[p].y;
        cellIdx2 = walls[p].x * vert + walls[p].y+1;
      }else{
        cellIdx1 =  walls[p].x * vert + walls[p].y;
        cellIdx2 = (walls[p].x + 1)*vert + walls[p].y;
      }

      if(cells.find(cellIdx1) != cells.find(cellIdx2)){
        if(walls[p].hv == 0){
          hWalls[walls[p].x][walls[p].y] = false;
        }else{
          vWalls[walls[p].x][walls[p].y] = false;
        }
        cells.union(cellIdx1, cellIdx2);
      }
    }
